...
// ─── Stats bar ────────────────────────────────────────────────────────────────
// Labels kept short (≤ 6 chars) so they never wrap on narrow screens.
  ...
  return (
    <div className="grid grid-cols-4 gap-2 mb-6">
      {items.map(({ label, value, color }) => (
        <div key={label} className="bg-muted/40 border rounded-xl px-2 py-3 text-center">
          <div className={`text-2xl font-black tabular-nums leading-none ${color}`}>{value}</div>
          <div className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wide mt-1 truncate px-0.5">
            {label}
          </div>
        </div>
      ))}
    ...
    <div className="flex gap-1 mb-4 bg-muted/40 border rounded-xl p-1">
      {tabs.map(({ key, label }) => (
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1 text-xs font-semibold py-1.5 px-1 rounded-lg transition-all
            ${active === key
...
function ServiceCard({ item, role, onDelete, onAssign }: {
  item: ServiceLog
  role: string | null
  ...
  return (
    <Card className="group relative hover:shadow-lg transition-all duration-200 border bg-card overflow-hidden">
      ...
      <CardContent className="p-5 pl-6">
          ...
          <div className="flex items-center gap-1 shrink-0">
            {isAdmin && item.status === 'pending' && !item.assigned_technician && (
              <Button
                ...
                onClick={() => onAssign(item.id)} title="Assign technician"
              >
        ...
        <h3 className="text-lg font-bold text-foreground leading-tight mb-3">{item.customer_name}</h3>
        ...
        {item.technician_notes && (
          <div className="mt-3 pt-3 border-t text-sm text-muted-foreground bg-muted/40 rounded-lg px-3 py-2">
            <span className="text-xs font-semibold uppercase tracking-wider text-foreground/70 block mb-1">Notes</span>
            {item.technician_notes}
          </div>
        )}
        ...
        {images.length > 0 && (
          <div className="mt-4 pt-3 border-t">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full">{images.length}</span>
            </div>
...
export default function Home() {
  ...
  // Admin    → all service_logs
  // Technician → only rows where assigned_technician = their email
  const fetchServices = useCallback(async (silent = false) => {
    ...
    if (!isAdmin) {
      // Resolve email: prefer profiles table, fall back to auth email
      ...
      const techEmail = profile?.email ?? user.email
      query = query.eq('assigned_technician', techEmail)
    }
    ...
    <div className="min-h-screen bg-background">
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <AlertTriangle className="h-5 w-5 text-destructive" /> Delete Service Log
            </AlertDialogTitle>
      ...
      <AlertDialog open={!!assignId} onOpenChange={open => { if (!open) { setAssignId(null); setSelectedTech('') } }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <UserPlus className="h-5 w-5" /> Assign Technician
            </AlertDialogTitle>
            <AlertDialogDescription>
          ...
          <Select value={selectedTechnician} onValueChange={setSelectedTech}>
            ...
            <SelectContent>
              ...
              {technicians.map(tech => (
                <SelectItem key={tech.id} value={tech.email}>{tech.email}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={confirmAssign} disabled={!selectedTechnician || isAssigning}>
