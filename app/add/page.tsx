...
import {
  ArrowLeft, Save, Calendar, Hash, User, Phone,
  MapPin, Tag, FileText, X, Loader2, AlertCircle,
  CheckCircle2, Building2, MailOpen,
} from 'lucide-react'
...
// ─── Helper: Pincode lookup (India Post API) ──────────────────────────────────
...
async function lookupPincode(pin: string): Promise<{ city: string } | null> {
  ...
  try {
    ...
    if (json?.[0]?.Status === 'Success') {
      ...
      return { city: postOffice?.District ?? postOffice?.Name ?? '' }
    }
...
export default function AddService() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Merge pincode into address for DB storage (keeps schema compatible)
      const addressFull = [
        formData.address,
        formData.city,
      ...
      if (selectedFiles.length > 0) {
        const { data: inserted, error: insertError } = await supabase
          .from('service_logs').insert([payload]).select('id').single()
        ...
        const { error: updateError } = await supabase
          .from('service_logs').update({ images: imageUrls }).eq('id', inserted.id)
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-2xl mx-auto px-4 py-6 pb-24">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl font-black text-foreground tracking-tight">New Service Entry</h1>
            ...
            <Card className="overflow-hidden">
              <CardContent className="p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <Field id="service_date" label="Service Date" icon={<Calendar className="h-3.5 w-3.5" />} required error={fieldErrors.service_date}>
                    <Input
                      id="service_date"
                      type="date"
                      required
              ...
              <CardContent className="p-5">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {/* Pincode first — auto-fills city */}
                    <Field id="pincode" label="Pincode" icon={<MailOpen className="h-3.5 w-3.5" />} error={fieldErrors.pincode}>
            ...
            <Card className="overflow-hidden">
              <CardContent className="p-5">
                ...
                <label
                  ...
                  className="flex flex-col items-center justify-center gap-2 w-full h-28 rounded-xl border-2 border-dashed border-muted-foreground/25 bg-muted/30 cursor-pointer hover:bg-muted/50 hover:border-muted-foreground/40 transition-all text-muted-foreground"
                >
                ...
                {previewUrls.length > 0 && (
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 mt-3">
                    {previewUrls.map((url, i) => (
                      <div key={i} className="relative rounded-xl overflow-hidden border bg-muted aspect-square">
